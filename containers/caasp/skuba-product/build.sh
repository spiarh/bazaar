...
retrieve_version() {
  local output=$(podman run --rm -ti registry.suse.com/suse/sle15 sh -c "zypper rr -a && zypper ar -G $REPO skuba-$REPO_ENV > /dev/null 2>&1 && zypper --no-color --no-gpg-checks info -r skuba-$REPO_ENV skuba")
