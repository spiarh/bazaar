...
db_convert_filecache_bigint() {
  echo "DB: Convert identifiers to BigInt on the filecache and activity table"
  echo "see: https://docs.nextcloud.com/server/latest/admin_manual/configuration_database/bigint_identifiers.html"
  php /var/www/html/occ -n db:convert-filecache-bigint
}

db_add_missing_indices() {
  echo "DB: Add missing indexes"
  php /var/www/html/occ -n db:add-missing-indices
}

if [ "$1" = "php-fpm7" ] || [ "${NEXTCLOUD_UPDATE:-0}" -eq 1 ]; then
    ...
    if version_greater "$image_version" "$installed_version"; then
        ...
        #upgrade
            ...
            echo "Upgrade nextcloud instance"
